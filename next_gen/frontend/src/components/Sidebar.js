  ...
  return (
    <div className="py-5">
      <div className="px-4 mb-3 flex items-center justify-between">
        ...
        <button
          onClick={openModal}
          className="bg-orange-200 rounded-full p-[2px] focus:outline-none focus:ring focus:border-orange-200 focus:ring-offset-1"
        >
          <svg
      ...
      <ul className="border-r border-gray-300 pr-2">
        {projects.map((project, index) => (
          <Link key={index} to={project._id} onClick={(e) => handleLocation(e)}>
            <li
              className={`px-5 py-1.5 mb-1 text-gray-600 font text-sm capitalize select-none hover:text-indigo-600 rounded transition-colors hover:bg-orange-200/80 ${
                paramsWindow === project._id &&
                "text-indigo-600 bg-orange-200/80"
              }`}
            >
              {project.title}
            </li>
